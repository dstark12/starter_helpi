...
export const GetResponse = async (
  apikey: string, 
  prompt: string, 
  result: (careers: string, aboutYou: string) => void
) => {
  /*
  ...
  const requestBody = {
    model: "gpt-4o",
    messages: [{role: 'system', content: 'You are an agent that assists people with finding career suggestions. You always respond in the exact format requested, without any additional comments.'}, { role: 'user', content: prompt }],
  };
  ...
  try {
    ...
    let aboutYou = response.substring(response.indexOf("About You:"), response.length);
    result(`${c1}|${c2}|${c3}`, aboutYou);
  } catch (error) {
